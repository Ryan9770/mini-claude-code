...
- 🩹 **로컬 모델 보완** — 반복 붕괴·의도 루프 감지 후 중단·복구, Windows Git Bash 라우팅
...
| `src/skills.ts` · `src/mcp.ts` · `src/evolve.ts` | 스킬 · MCP · 자기개선 |
...
| `MCC_SKILLS_DIR` | `~/.mcc/skills` | 스킬 디렉터리 |
| `MCC_HOME` | `~/.mcc` | 스킬·MCP·learnings·로그 저장 위치 |
...
Anthropic `SKILL.md` 형식 호환. `~/.mcc/skills/<name>/SKILL.md` 에 폴더를 넣으면 인식됩니다.
...
- 형식:
  ...
  ---
  name: skill-name
  description: 이 스킬이 무엇을 하는지 한 줄 (트리거 판단용)
  ---
  # 지침 본문
  ```
