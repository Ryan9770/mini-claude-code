...
- 🧰 **도구 세트** — read / write / edit / list / make_dir / glob / grep / run_command / use_skill
...
- 🎯 **스킬 라우터** — 프롬프트를 읽고 대규모 스킬 라이브러리(harness-100 등)에서 관련 스킬만 동적 선택(어휘 검색 + 점수 게이트) → 소형 모델의 스킬 과부하 방지
...
| `MCC_SKILL_LIB` | `<프로젝트>/harness-100/ko` | 대규모 스킬 라이브러리(라우터가 동적 선택). 미존재 시 라우터 비활성 |
...
로컬 스킬(상시 로드)과 별개로, 수백 개짜리 스킬 라이브러리(`<프로젝트>/harness-100/ko` 등)를 두면 **매 작업마다 프롬프트를 읽고 관련 스킬만 골라** 노출합니다.
...
- 포함 라이브러리 예: [revfactory/harness-100](https://github.com/revfactory/harness-100)(Apache-2.0)의 한국어 스킬 세트
